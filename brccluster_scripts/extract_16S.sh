...
for species in endosymb_only/genomes/*; do
	...
	for gff in "$species"/*.gff; do
		...
		grep '16S ribosomal RNA' $gff_file | awk '$3 == "rRNA" {print $1 "\t" $4-1 "\t" $5}' > $outfile

		[ -s $outfile ] || continue
		genome_file=endosymb_only/genomes/$sp_name/${filename}.fna
