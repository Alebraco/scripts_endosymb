...
# Worker script for downloading and extracting ATB genome assemblies.
# Called as a SLURM array job by submit_atb_assemblies.sh — do not run directly.
#
# Each task handles one assembly tar.xz batch:
#   1. Look up this task's tar filename and URL from the batch index.
#   2. Download the tar.xz (skipped if already present).
#   3. For each genome in this batch's list, extract only that file from the tar.
#   4. Delete the tar.xz after extraction to save disk space.
#
# Output: genomes are written to GENOMES_DIR/Species_name/SAMPLE.fa
#
# SLURM_ARRAY_TASK_ID is set automatically by SLURM and corresponds to the
# 1-based line number in asm_batch_index.tsv.

# Must match the OUTDIR set in submit_atb_assemblies.sh
OUTDIR="atb_bacteria"

BATCH_INDEX="${OUTDIR}/asm_batch_index.tsv"   # idx → tar_xz, url
BATCH_LISTS="${OUTDIR}/batch_lists"            # per-batch genome lists
DL_DIR="${OUTDIR}/downloads"                   # temporary tar.xz storage
GENOMES_DIR="${OUTDIR}/genomes"               # final genome FASTA output

mkdir -p "${DL_DIR}" "${GENOMES_DIR}"
...
# Per-batch list of genomes to extract.
# Columns: sample, sylph_species, path_of_fasta_inside_tar
BATCH_LIST="${BATCH_LISTS}/asm_batch_${BATCH_IDX}.tsv"
EXTRACTED=0
SKIPPED=0

while IFS=$'\t' read -r SAMPLE SPECIES FILE_IN_TAR; do
    # Build a safe output directory name from the species string
    SPECIES_DIR="${GENOMES_DIR}/$(echo "${SPECIES}" | tr ' ' '_' | tr '/' '_' | tr -d "'")"
    DEST="${SPECIES_DIR}/${SAMPLE}.fa"
    ...
    # Selective extraction: pipe only this file's bytes directly to the dest,
    # avoiding writing the rest of the tar to disk (-O = stdout).
    tar -xJf "${TARBALL}" "${FILE_IN_TAR}" -O > "${DEST}" 2>/dev/null
    if [[ $? -eq 0 && -s "${DEST}" ]]; then
