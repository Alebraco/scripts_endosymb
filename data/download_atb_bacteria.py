#!/usr/bin/env python3
"""
Download bacterial genomes and Bakta annotations from AllTheBacteria (ATB).

Mirrors the sampling logic of download_ncbi_bacteria.py:
  - Filter out unknown species and non-standard genetic code genera
  - Sample N genomes per species (default 10)

Produces a manifest and batch lists for parallel download and selective
extraction on HPC. Submit jobs with data/submit_atb_assemblies.sh and
data/submit_atb_bakta.sh.

Usage:
  python download_atb_bacteria.py \
      --file-list file_list.all.202505.tsv \
      --bakta-status atb.bakta.r0.2.status.tsv \
      --batch-urls batch_urls.tsv \
      --outdir atb_bacteria \
      --max-per-species 10

Input files:
  file_list.all.202505.tsv   — ATB assembly manifest (one row per genome,
                               includes sample ID, sylph_species, tar path)
  atb.bakta.r0.2.status.tsv — Bakta annotation status per sample; only
                               samples with status=PASS are kept
  batch_urls.tsv             — Two-column TSV: tar_xz filename, download URL
                               for each Bakta batch archive

Output layout (inside --outdir):
  manifest.tsv               — Final selected samples with all relevant paths
  asm_batch_index.tsv        — Index for SLURM assembly array: idx, tar, url
  bkt_batch_index.tsv        — Index for SLURM bakta array: idx, tar, url
  batch_lists/
    asm_batch_N.tsv          — Per-batch lists of genomes to extract
    bkt_batch_N.tsv          — Per-batch lists of annotations to extract
  logs/                      — Created for SLURM log output
"""

import os
import re
import argparse
import pandas as pd


# Genera that use non-standard genetic codes (UGA = Trp instead of Stop).
# Excluded because codon usage analyses assume the standard code.
NON_STANDARD_CODE_GENERA = {
    "Mycoplasma", "Spiroplasma", "Ureaplasma", "Phytoplasma",
    "Acholeplasma", "Mesoplasma", "Entomoplasma",
}


def safe_dirname(name):
    """Convert species name to a safe directory name."""
    return name.strip().replace(" ", "_").replace("/", "_").replace("'", "")


def strip_gtdb_suffix(genus):
    """Remove GTDB-style suffixes (e.g. _D, _A) appended to genus names
    to indicate polyphyletic groups in the GTDB taxonomy."""
    return re.sub(r"_[A-Z]$", "", genus)


def load_file_list(path):
    """Load the ATB assembly manifest TSV."""
    print(f"Loading assembly file list: {path}")
    df = pd.read_csv(path, sep="\t", low_memory=False)
    print(f"  Total entries: {len(df):,}")
    return df


def load_bakta_status(path):
    """Load the Bakta annotation status TSV."""
    print(f"Loading Bakta status: {path}")
    df = pd.read_csv(path, sep="\t", low_memory=False)
    print(f"  Total entries: {len(df):,}")
    return df


def load_batch_urls(path):
    """Load the Bakta batch download URL table (tar_xz filename → URL)."""
    print(f"Loading batch URLs: {path}")
    df = pd.read_csv(path, sep="\t", header=None, names=["tar_xz", "url"])
    print(f"  Total batches: {len(df):,}")
    return df


def filter_and_select(file_list, bakta_status, batch_urls, max_per_species):
    """Apply quality filters and select up to max_per_species genomes per species.

    Filtering steps (applied in order):
      1. Drop samples with unknown sylph species assignment.
      2. Drop non-standard genetic code genera (see NON_STANDARD_CODE_GENERA).
      3. Inner-join with Bakta status; keep only samples that passed annotation.
      4. Attach Bakta batch download URLs; drop samples with no URL.
      5. Take the first max_per_species rows per species (preserves file order).
    """

    df = file_list.copy()
    n_start = len(df)

    # 1. Remove samples where sylph could not assign a species
    df = df[df["sylph_species"] != "unknown"].copy()
    print(f"  After removing unknown species:     {len(df):,} / {n_start:,}")

    # 2. Remove non-standard genetic code genera.
    # Strip GTDB suffixes before matching so e.g. "Mycoplasma_A" is caught.
    def is_nonstandard(species_name):
        genus = str(species_name).split()[0]
        return strip_gtdb_suffix(genus) in NON_STANDARD_CODE_GENERA

    mask = df["sylph_species"].apply(is_nonstandard)
    df = df[~mask].copy()
    print(f"  After removing non-std code genera: {len(df):,} (removed {mask.sum():,})")

    # 3. Keep only samples with a successful Bakta annotation (status == PASS).
    # The join also attaches the bakta tar_xz filename for each sample.
    bakta_pass = bakta_status[bakta_status["status"] == "PASS"][["sample", "tar_xz"]].copy()
    bakta_pass = bakta_pass.rename(columns={"tar_xz": "bakta_tar_xz"})
    df = df.merge(bakta_pass, on="sample", how="inner")
    print(f"  After joining with PASS bakta:      {len(df):,}")

    # 4. Map each sample's bakta tar_xz to its download URL.
    # Samples whose batch tar has no URL in batch_urls.tsv are dropped.
    url_map = dict(zip(batch_urls["tar_xz"], batch_urls["url"]))
    df["bakta_url"] = df["bakta_tar_xz"].map(url_map)
    n_no_url = df["bakta_url"].isna().sum()
    if n_no_url > 0:
        print(f"  WARNING: {n_no_url:,} samples have no bakta batch URL — dropping them")
        df = df.dropna(subset=["bakta_url"])

    # 5. Take the first N rows per species.
    # groupby preserves the original row order within each group (sort=False),
    # so this effectively takes the first N samples encountered in the manifest.
    df = (
        df.groupby("sylph_species", sort=False)
          .head(max_per_species)
          .reset_index(drop=True)
    )
    n_species = df["sylph_species"].nunique()
    print(f"  After {max_per_species} per species:          {len(df):,} samples across {n_species:,} species")

    return df


def write_manifest(df, outdir):
    """Write the full selected-sample manifest to outdir/manifest.tsv.

    Columns: sample, sylph_species, filename_in_tar_xz (path inside assembly
    tar), tar_xz (assembly batch), tar_xz_url (assembly download URL),
    bakta_tar_xz (bakta batch), bakta_url (bakta download URL).
    """
    manifest_path = os.path.join(outdir, "manifest.tsv")
    cols = ["sample", "sylph_species", "filename_in_tar_xz", "tar_xz", "tar_xz_url",
            "bakta_tar_xz", "bakta_url"]
    df[cols].to_csv(manifest_path, sep="\t", index=False)
    print(f"\nManifest written: {manifest_path} ({len(df):,} samples)")
    return manifest_path


def write_batch_lists(df, outdir):
    """Write per-batch extraction lists and SLURM index files.

    ATB archives samples into large tar.xz batches. Rather than downloading
    every batch in full, the SLURM jobs do selective extraction — they pull
    only the files that appear in the per-batch list.

    Assembly output (batch_lists/asm_batch_N.tsv):
      sample  sylph_species  filename_in_tar_xz
    Each row is one genome to extract from batch N's assembly tar.

    Bakta output (batch_lists/bkt_batch_N.tsv):
      sample  sylph_species
    Each row is one sample whose .bakta.json to extract from batch N's tar.

    Index files (asm_batch_index.tsv / bkt_batch_index.tsv):
      batch_idx  tar_xz_filename  download_url
    These are read by the SLURM array jobs to look up which tar to download
    for a given array task ID.
    """
    batch_dir = os.path.join(outdir, "batch_lists")
    os.makedirs(batch_dir, exist_ok=True)

    # --- Assembly batches ---
    # Group selected samples by which assembly tar.xz they came from.
    asm_batches = {}
    for idx, (tar_xz, group) in enumerate(df.groupby("tar_xz"), start=1):
        asm_batches[idx] = {
            "tar_xz": tar_xz,
            "url": group["tar_xz_url"].iloc[0],
            "files": group["filename_in_tar_xz"].tolist(),
            "samples": group["sample"].tolist(),
            "species": group["sylph_species"].tolist(),
        }
        list_path = os.path.join(batch_dir, f"asm_batch_{idx}.tsv")
        group[["sample", "sylph_species", "filename_in_tar_xz"]].to_csv(
            list_path, sep="\t", index=False, header=False
        )

    # --- Bakta batches ---
    # Group by bakta tar_xz (separate from assembly tars).
    bkt_batches = {}
    for idx, (tar_xz, group) in enumerate(df.groupby("bakta_tar_xz"), start=1):
        bkt_batches[idx] = {
            "tar_xz": tar_xz,
            "url": group["bakta_url"].iloc[0],
            "samples": group["sample"].tolist(),
            "species": group["sylph_species"].tolist(),
        }
        list_path = os.path.join(batch_dir, f"bkt_batch_{idx}.tsv")
        group[["sample", "sylph_species"]].to_csv(
            list_path, sep="\t", index=False, header=False
        )

    # Write index files: one row per batch, consumed by SLURM array jobs.
    # The array task ID (1-based) maps directly to the row number here.
    asm_index_path = os.path.join(outdir, "asm_batch_index.tsv")
    with open(asm_index_path, "w") as f:
        for idx, info in sorted(asm_batches.items()):
            f.write(f"{idx}\t{info['tar_xz']}\t{info['url']}\n")

    bkt_index_path = os.path.join(outdir, "bkt_batch_index.tsv")
    with open(bkt_index_path, "w") as f:
        for idx, info in sorted(bkt_batches.items()):
            f.write(f"{idx}\t{info['tar_xz']}\t{info['url']}\n")

    print(f"Assembly batches: {len(asm_batches)} unique tar.xz files")
    print(f"Bakta batches:    {len(bkt_batches)} unique tar.xz files")
    print(f"Batch lists written to: {batch_dir}/")

    return asm_batches, bkt_batches


def main():
    parser = argparse.ArgumentParser(
        description="Select and prepare ATB genome + annotation downloads."
    )
    parser.add_argument(
        "--outdir", default="atb_bacteria",
        help="Root output directory (default: atb_bacteria)"
    )
    parser.add_argument(
        "--max-per-species", type=int, default=10,
        help="Maximum genomes per species (default: 10)"
    )
    parser.add_argument(
        "--file-list", required=True,
        help="Path to file_list.all.202505.tsv (assembly list with species)"
    )
    parser.add_argument(
        "--bakta-status", required=True,
        help="Path to atb.bakta.r0.2.status.tsv (bakta annotation status)"
    )
    parser.add_argument(
        "--batch-urls", required=True,
        help="Path to batch_urls.tsv (bakta batch download URLs)"
    )
    args = parser.parse_args()

    # Create output directories
    os.makedirs(args.outdir, exist_ok=True)
    os.makedirs(os.path.join(args.outdir, "logs"), exist_ok=True)

    # Load data
    file_list = load_file_list(args.file_list)
    bakta_status = load_bakta_status(args.bakta_status)
    batch_urls = load_batch_urls(args.batch_urls)

    # Filter and select
    print("\nFiltering and selecting samples:")
    selected = filter_and_select(file_list, bakta_status, batch_urls, args.max_per_species)

    # Write outputs
    write_manifest(selected, args.outdir)
    write_batch_lists(selected, args.outdir)

    print(f"\n{'='*60}")
    print(f"Next steps:")
    print(f"  1. Transfer {args.outdir}/ to your HPC cluster")
    print(f"  2. Set OUTDIR in data/submit_atb_assemblies.sh and data/submit_atb_bakta.sh")
    print(f"  3. sbatch data/submit_atb_assemblies.sh")
    print(f"  4. sbatch data/submit_atb_bakta.sh")
    print(f"{'='*60}")


if __name__ == "__main__":
    main()
